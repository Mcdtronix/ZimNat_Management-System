...
export default function Inbox() {
  ...
  const isUnderwriter =
    perms?.user_type === "underwriter" || perms?.user_type === "manager";
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      quotations: all.filter((n: Notification) => n.type === "quotation")
        .length,
  ...
  const markAllRead = useMutation({
    mutationFn: () =>
      api("/api/notifications/mark_all_read/", { method: "POST" }),
    onSuccess: () => {
  ...
  const autoQuote = useMutation({
    mutationFn: (policyId: number) =>
      api(`/api/policies/${policyId}/auto_quote/`, { method: "POST" }),
    onSuccess: () => {
  ...
  const previewAutoQuote = useMutation({
    mutationFn: (policyId: number) =>
      api(`/api/policies/${policyId}/auto_quote/?preview=true`, {
        method: "POST",
      }),
  });
  ...
  const acceptQuote = useMutation({
    mutationFn: (quotationId: number) =>
      api(`/api/quotations/${quotationId}/accept/`, { method: "POST" }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["notifications"] }),
  });
  const declineQuote = useMutation({
    mutationFn: (quotationId: number) =>
      api(`/api/quotations/${quotationId}/decline/`, { method: "POST" }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["notifications"] }),
  });
  ...
  async function downloadQuote(
    ...
    const token = getAuthToken();
    const url = `${API_BASE}/api/policies/${policyId}/export_quote/?format=${fmt}${
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="policy_application">
                    Policy Applications
                  </SelectItem>
                  ...
                  <div key={n.id}>
                    ...
                    >
                      <div className="flex-1">
                        ...
                        {n.type === "quotation" && (
                          <div className="mt-2 text-sm space-y-1">
                            <div>
                              ...
                              {n.payload?.policy_number}
                            </div>
                            <div className="flex gap-4">
                              <div>
                                <span className="font-medium">Premium:</span>{" "}
                                {n.payload?.currency || "USD"}{" "}
                                {n.payload?.premium_amount}
                              </div>
                              <div>
                                <span className="font-medium">Cover:</span>{" "}
                                {n.payload?.currency || "USD"}{" "}
                                {n.payload?.coverage_amount}
                              </div>
                            </div>
                            {n.payload?.terms && (
                            ...
                            {n.payload?.bank_details && (
                              <div className="text-muted-foreground">
                                ...
                                {n.payload.bank_details.bank} •{" "}
                                {n.payload.bank_details.account_number} •{" "}
                                {n.payload.bank_details.branch}
                              </div>
                            )}
                            <div className="pt-1">
                        ...
                        {n.type === "policy_application" && isUnderwriter && (
                          <div className="mt-2 text-sm space-y-1">
                            <div>
                              <span className="font-medium">
                                Policy Application:
                              </span>{" "}
                              {n.payload?.policy_number || "New Application"}
                            </div>
                            <div className="flex gap-4">
                              <div>
                              ...
                              <div>
                                ...
                                {n.payload?.vehicle_number}
                              </div>
                            </div>
                            {n.payload?.coverage_type && (
                              <div>
                            ...
                            <div className="pt-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <Link
                                  to={`/underwriter/quotes?policy=${n.payload?.policy_id}`}
                                >
                                  <Button
                                    size="sm"
                                    variant="default"
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    toast.success(
                                      "Marked as read - you can process this later from the Policies section"
                                    );
                                  }}
                        ...
                        {n.type === "payment_success" && isUnderwriter && (
                          <div className="mt-2 text-sm space-y-1">
                            <div>
                              <span className="font-medium">
                                Payment Received:
                            ...
                            <div>
                              ...
                              {n.payload?.payment_method
                                ?.replace("_", " ")
                                .toUpperCase()}
                            </div>
                            {n.payload?.payment_method === "bank_transfer" && (
                              <div className="pt-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                  <Link
                                    to={`/underwriter/payments?payment=${n.payload?.payment_id}`}
                                  >
                                    <Button
                                      size="sm"
                                      variant="default"
                                  ...
                                  <Button
                                    ...
                                    onClick={(e) => {
                                      ...
                                      markRead.mutate(n.id);
                                      toast.success(
                                        "Marked as read - you can process this later from the Payments section"
                                      );
                                    }}
                                    disabled={markRead.isPending}
                                  >
          ...
          <DialogContent className="max-w-2xl">
            ...
            {selected && (
              <div className="space-y-4">
                ...
                {isUnderwriter &&
                  selected.type === "status_update" &&
                  selected.payload?.policy_id &&
                  selected.payload?.status === "pending" && (
                    <div className="rounded-md border p-3 space-y-2">
                      <div className="font-medium">
                        Auto calculate subscription
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Preview calculation before sending a quotation to the
                        customer. Sending is enabled after preview.
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            previewAutoQuote.mutate(selected.payload.policy_id);
                          }}
                          disabled={previewAutoQuote.isPending}
                        >
                          {previewAutoQuote.isPending
                            ? "Previewing…"
                            : "Preview calculation"}
                        </Button>
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            autoQuote.mutate(selected.payload.policy_id);
                          }}
                          disabled={
                            autoQuote.isPending || !previewAutoQuote.data
                          }
                        >
                        ...
                        <Button
                          ...
                          disabled={!previewAutoQuote.data}
                        >
                          Export PDF (preview)
                        </Button>
                        <Button
                      ...
                      {previewAutoQuote.data && (
                        <div className="text-sm grid grid-cols-2 gap-x-6 gap-y-1 pt-2">
                          <div>
                            ...
                            USD {previewAutoQuote.data.annual_premium}
                          </div>
                          <div>
                            <span className="text-muted-foreground">
                              Termly premium:
                            </span>{" "}
                            USD {previewAutoQuote.data.termly_premium}
                          </div>
                          <div>
                            <span className="text-muted-foreground">
                              Sum insured:
                            </span>{" "}
                            USD {previewAutoQuote.data.coverage_amount}
                          </div>
                          <div className="col-span-2 text-muted-foreground">
                            {previewAutoQuote.data.terms}
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="rounded-md border p-3 space-y-2">
                    ...
                    <div className="flex justify-end">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          sendManualMessage.mutate({
                            policyId: selected.payload.policy_id,
                            title: manualTitle,
                            message: manualBody,
                          });
                        }}
                        disabled={
                          sendManualMessage.isPending || !manualBody.trim()
                        }
                      >
                  ...
                  <div className="rounded-md border p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <Input
                        ...
                        placeholder="Assessor phone (optional)"
                      />
                      <Input
                        type="date"
                        value={visitDate}
                    ...
                    <div className="flex justify-end">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          if (selected?.payload?.claim_id) {
                            sendAssessor.mutate({
                              claimId: selected.payload.claim_id,
                              assessor_name: assessorName,
                              assessor_phone: assessorPhone || undefined,
                              visit_date: visitDate,
                              message: manualBody || undefined,
                            });
                          }
                        }}
                        disabled={
                          sendAssessor.isPending ||
                          !assessorName.trim() ||
                          !visitDate
                        }
                      >
                ...
                {selected.type === "quotation" && (
                  <div className="rounded-md border p-3 space-y-1 text-sm">
                    <div>
                      <span className="font-medium">Policy:</span>{" "}
                      {selected.payload?.policy_number}
                    </div>
                    <div className="flex gap-6">
                      <div>
                        <span className="text-muted-foreground">Premium:</span>{" "}
                        {selected.payload?.currency || "USD"}{" "}
                        {selected.payload?.premium_amount}
                      </div>
                      <div>
                        <span className="text-muted-foreground">Cover:</span>{" "}
                        {selected.payload?.currency || "USD"}{" "}
                        {selected.payload?.coverage_amount}
                      </div>
                    </div>
                    {selected.payload?.terms && (
                      <div className="text-muted-foreground">
                        {selected.payload.terms}
                      </div>
                    )}
